...
struct adb_dev {
	...
	int rx_done;
	bool notify_close; //added by songxh for patch:USB: android: Don't disable configuration for every adb close
	bool close_notified;//added by songxh for patch: Fix adb device file closing bug
};
...
requeue_req:
    ...
    /*added by songxh for patch: USB: android: Wakeup both reader and writer processes upon TX/RX error    begain*/
    if (atomic_read(&dev->error))
	     wake_up(&dev->write_wq);
	/*added by songxh for patch: USB: android: Wakeup both reader and writer processes upon TX/RX error    end*/	 
	adb_unlock(&dev->read_excl);
